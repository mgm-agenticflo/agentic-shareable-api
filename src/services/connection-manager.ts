 ...
 */
export const CreateDynamoDBConnectionManager = (docClient: DynamoDBDocumentClient, TableName: string) => ({
  /**
   ...
   */
  getConnection: async (connectionId: string): Promise<ConnectionRecord | null> => {
   ...
   */
  getConnectionsByChannel: async (channelId: string): Promise<ConnectionRecord[]> => {
    const result = await docClient.send(
      new ScanCommand({
        TableName,
   ...
   */
  getConnectionsByResourceId: async (resourceType: string, resourceId: string): Promise<ConnectionRecord[]> => {
    const result = await docClient.send(
      new ScanCommand({
        TableName,
...
export const instance = isTrue(process.env.IS_OFFLINE)
  ? CreateInMemoryConnectionManager(new Map<string, ConnectionRecord>())
