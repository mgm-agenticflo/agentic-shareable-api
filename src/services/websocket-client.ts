...
export const CreateAWSWebSocketClient = () => {
   ...
   */
  const getClient = (endpoint: string): ApiGatewayManagementApiClient => {
    if (!client) {
  ...
  return {
     ...
     */
    disconnect: async (endpoint: string, connectionId: string): Promise<void> => {
      try {
        const apiClient = getClient(endpoint);
        await apiClient.send(
...
export const websocketClient: WebSocketClient = isTrue(process.env.IS_OFFLINE)
