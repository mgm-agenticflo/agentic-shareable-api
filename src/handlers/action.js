...
exports.execute = async (event) => {
  try {
    ...
    try {
      tokenPayload = coreApi.transientTokenService.verifyTransientToken(transientToken);
    } catch (err) {
      // Token expired, try to refresh
      console.log('Transient token expired, attempting refresh...');
      try {
        tokenPayload = coreApi.transientTokenService.verifyTransientToken(transientToken, { ignoreExpiration: true });
        ...
        if (!configData.success) {
          return error('Shareable token invalid', 403, 'SHAREABLE_TOKEN_INVALID');
        }
        ...
        return error(`Token refresh failed: ${refreshErr.message}`, 403, 'TOKEN_REFRESH_FAILED');
      }
    }
    ...
    try {
      switch (action) {
        case 'chat':
          console.log('Calling chat API:', `/chats/webchat/${shareableToken}`);
          ...
          result = await chatClient.post(`/chats/webchat/${shareableToken}`, payload);
          break;
          
        case 'history':
          console.log('Calling history API:', `/chats/webchat/${shareableToken}/history`);
          result = await coreApi.client.get(`/chats/webchat/${shareableToken}/history?sessionId=${payload.sessionId}`);
          break;
