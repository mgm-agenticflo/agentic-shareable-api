...
async function handleMessage(ws, connectionId, message) {
    ...
    try {
        ...
        if (!connection) {
            ...
            await websocketClientNative.sendToConnection(endpoint, connectionId, '', wsFailure('Connection not found', HttpStatusCode.NotFound));
            return;
        }
        ...
        if (!handler) {
            ...
            await websocketClientNative.sendToConnection(endpoint, connectionId, command, wsFailure(`Unknown command: ${command}`, HttpStatusCode.BadRequest));
            return;
        }
        ...
        await websocketClientNative.sendToConnection(endpoint, connectionId, command, wsSuccess(handlerResponse.result, handlerResponse.statusCode));
    }
    catch (err) {
        ...
        try {
            ...
            if (err instanceof HttpCodedError) {
                await websocketClientNative.sendToConnection(endpoint, connectionId, command, wsFailure(err.message, err.statusCode));
                ...
                await websocketClientNative.sendToConnection(endpoint, connectionId, command, wsFailure('Internal server error', HttpStatusCode.InternalServerError));
            }
        }
        catch (sendError) {
