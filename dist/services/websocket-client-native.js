...
export const CreateNativeWebSocketClient = () => {
    return {
         ...
         */
        sendToConnection: (endpoint, connectionId, command, data) => {
         ...
         */
        sendToConnections: (endpoint, connectionIds, command, data) => {
            const staleConnections = [];
            return Promise.all(connectionIds.map((connectionId) => {
                try {
                    const ws = connections.get(connectionId);
                    if (!ws || ws.readyState !== WebSocket.OPEN) {
         ...
         */
        broadcast: (endpoint, connectionIds, command, data) => {
            const staleConnections = [];
            return Promise.all(connectionIds.map((connectionId) => {
                try {
                    const ws = connections.get(connectionId);
         ...
         */
        disconnect: (endpoint, connectionId) => {
            return Promise.resolve().then(() => {
                try {
                    const ws = connections.get(connectionId);
                    if (ws) {
...
//# sourceMappingURL=websocket-client-native.js.map