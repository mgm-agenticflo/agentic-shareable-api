 ...
 */
export function websocketToRequestEvent(ws, message, connectionData, connectionId) {
 ...
 */
export function createWebSocketConnectEvent(connectionId) {
 ...
 */
export function createWebSocketDisconnectEvent(connectionId) {
    const now = Math.floor(Date.now() / 1000);
    return {
        requestContext: {
            connectionId,
